	...
	case 'openSideCraft':
		...
		if(canCraft) {
			$('#craft-button-div').html(`
				<button type="button" id="craft-button" data-item="${event.data.itemNameID}" data-recipe="${event.data.recipe}" data-amount="${event.data.itemAmount}" onclick="craft(this)" class="btn btn-blue flex-grow-1 mt-2" style="border-radius: 10px; flex-basis: 100%; width: 100%; font-weight: 600;"><i class="fas fa-pencil-ruler"></i> ประดิษฐ์  ${event.data.itemName}</button>
			`);
			$('#craft-button').fadeIn();
		} else {
