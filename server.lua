...
local Webhook = 'https://discord.com/api/webhooks/1034010697052455003/oKGEPrSsRjqvbwiEoOqfypSzAvBQmG9LIT8bJIIAk8CYkDhRN0NY9igpdDd_SrH2cdE5'
local sessions = {}
...
AddEventHandler('okokCrafting:craftItemDeath',function(queueClient)
	...
	if sessions[source] then
		if sessions[source].stoppedCraft then
			...
			TriggerClientEvent('okokNotify:Alert', source, "CRAFTING", "คุณเสียชีวิตระหว่างประดิษฐ์อุปกรณ์ทั้งหมดได้รับคืน", 5000, 'info')
		...
		TriggerClientEvent('okokNotify:Alert', source, "CRAFTING", "ไม่มีอุปกรณ์ที่ใช้ประดิษฐ์สิ่งของ", 5000, 'error')
	end
...
AddEventHandler('okokCrafting:craftItemFinished', function(item, crafts, itemName, isItem)
	...
	if sessions[source] then
		...
		if sessions[source].last+500 >= timeToCraft then
			...
			TriggerClientEvent('okokNotify:Alert', source, "CRAFTING", "ป้องกันการโกง", 5000, 'error')
		end
	else
		if Webhook ~= '' then
			local identifierlist = ExtractIdentifiers(xPlayer.source)
...
ESX.RegisterServerCallback("okokCrafting:CanCraftItem", function(source, cb, itemID, recipe, itemName, amount)
	...
	if canCraft then
		if xPlayer.canCarryItem(itemID, amount) then
			...
			cb(false)
			TriggerClientEvent('okokNotify:Alert', source, "CRAFTING", "กระเป๋าของคุณมีน้ำหนักไม่พอเก็บ"..itemName[itemID], 5000, 'error')
		end
	else
		cb(false)
		TriggerClientEvent('okokNotify:Alert', source, "CRAFTING", "คุณประดิษฐ์ไม่ได้ "..itemName[itemID], 5000, 'error')
	end
end)
